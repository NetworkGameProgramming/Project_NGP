...
bool GreenMush::Initialize()
{
	m_Info = GAMEOBJINFO{ 1600, 1240, 60, 60 };
	m_CollideInfo = GAMEOBJINFO{ 0, 0, 60, 60 };
	m_Speed = 0.f;
	m_RenderType = RENDER_OBJ;
	m_Direction = DIR_RIGHT;
	m_SpriteInfo.key = L"greenmush_right";
...
int GreenMush::Update_Input(const float& TimeDelta)
{
#ifdef CLIENT_MODE
...
int GreenMush::Update(const float& TimeDelta)
{
...
int GreenMush::Update_Position(const float& TimeDelta, const DIRECTION& Direction)
{
...
int GreenMush::Update_Sprite(const float& TimeDelta)
{
	...
	switch (m_Direction)
	{
	case DIR_LEFT: m_SpriteInfo.key = L"greenmush_left"; break;
	case DIR_RIGHT: m_SpriteInfo.key = L"greenmush_right"; break;
	}
	if (m_SpriteInfo.CurState == Monster_Die)
	{
		if ((float)m_SpriteInfo.MaxFrame <= m_SpriteInfo.SpriteIndex)
...
void GreenMush::Release()
{
}

void GreenMush::StateChange()
{
